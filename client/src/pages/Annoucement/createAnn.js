...
const CreateAnnoucement = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios({
                url: createAnnoucement.url,
                method: createAnnoucement.method,
                data: {
                    ...formData,
            ...
            if (response.status === 200) {
                toast.success('Thông báo được tạo thành công!');
                setFormData({
    ...
    const handleBackAnnoucement = () => {
        navigate('/annouce');
    };

    return (
        <div className="tong rounded-md p-4 mb-2 w-full relative">
            <h1 className="text-2xl font-bold mb-4 text-center font-larger">Tạo thông báo mới </h1>
            <Button
                onClick={handleBackAnnoucement}
                primary
